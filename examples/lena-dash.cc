 ...
 *
 * Edited by: Dimitrios J. Vergados <djvergad@gmail.com>
 */
    ...
    for (NodeList::Iterator it = NodeList::Begin(); it != NodeList::End(); ++it)
    {
        ...
        for (int j = 0; j < nDevs; j++)
        {
            ...
            if (enbdev)
            {
                Vector pos = node->GetObject<MobilityModel>()->GetPosition();
                outFile
                    << "set label \"" << enbdev->GetCellId() << "\" at " << pos.x << "," << pos.y
...
static ns3::GlobalValue g_outdoorUeMaxSpeed(
    ...
    ns3::DoubleValue(0.0),
    ns3::MakeDoubleChecker<double>());
static ns3::GlobalValue g_targetDt(
    "targetDt",
...
static ns3::GlobalValue g_window("window",
                                 ...
                                 ns3::MakeDoubleChecker<double>());
static ns3::GlobalValue g_algorithms(
    "algorithms",
...
main(int argc, char* argv[])
{
    ...
    GlobalValue::GetValueByName("useDash", booleanValue);
    bool useDash = booleanValue.Get();
    ...
    GlobalValue::GetValueByName("targetDt", doubleValue);
    double targetDt = doubleValue.Get();
    GlobalValue::GetValueByName("window", doubleValue);
    ...
    if (epc)
    {
        ...
        for (uint32_t u = 0; u < ues.GetN(); ++u)
        {
            ...
            for (uint32_t b = 0; b < numBearersPerUe; ++b)
            {
                ...
                {
                    NS_LOG_LOGIC("installing DASH client app on UE "
                                 << u << ", using " << algorithms[u % algorithms.size()]);
                    DashClientHelper dashClientHelper("ns3::TcpSocketFactory",
                                                      InetSocketAddress(remoteHostAddr, dashPort),
                                                      algorithms[u % algorithms.size()]);

                    dashClientHelper.SetAttribute(
                        "VideoId",
                        UintegerValue(u + 1)); // VideoId should be positive
                ...
                else // use TCP
                {
                    if (epcDl)
                    {
                        ...
                        dlClientHelper.SetAttribute("MaxBytes", UintegerValue(10000));
                        clientApps.Add(dlClientHelper.Install(remoteHost));
                        PacketSinkHelper dlPacketSinkHelper(
                            "ns3::TcpSocketFactory",
                            InetSocketAddress(Ipv4Address::GetAny(), dlPort));
                    ...
                    if (epcUl)
                    {
                        ...
                        ulClientHelper.SetAttribute("MaxBytes", UintegerValue(10000));
                        clientApps.Add(ulClientHelper.Install(ue));
                        PacketSinkHelper ulPacketSinkHelper(
                            "ns3::TcpSocketFactory",
                            InetSocketAddress(Ipv4Address::GetAny(), ulPort));
    ...
    for (k = 0; k < users; k++)
    {
        ...
        std::cout << algorithms[k % algorithms.size()] << "-Node: " << k;
        app->GetStats();
    }
